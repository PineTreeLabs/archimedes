...
def qpsol(
    ...
    **options,
    ...
    where ``x`` represents decision variables.
    ...
    ----------
    obj : callable
        Objective function to minimize with signature ``obj(x)``.
        Must return a scalar value.
    constr : callable
        Constraint function with signature ``constr(x)``.
        Must return a vector of constraint values where the constraints
        are interpreted as ``lba <= constr(x) <= uba``.
    x0 : array-like
        Initial guess for the decision variables. This is used to determine
        the linearization point for the convex approximation. If None, the
        initial guess will be set to zero.
    lbg : array-like, optional
        Lower bounds for the constraints. If None, the lower bounds will be
        set to negative infinity.
    ubg : array-like, optional
    ...
    lam_a0 : array-like, optional
        ...
        This is used to warm-start the QP solver. Not used if None.
    x_guess : array-like, optional
        Initial guess for the decision variables. This is used to warm-start
        the QP solver. Not used if None.
    ...
    **options : dict
    ...
    lbg_sym = sym_like(lbg, name="lbg", kind="MX")
    ubg_sym = sym_like(ubg, name="ubg", kind="MX")
    f = obj(x)
    g = constr(x)

    # Setup for evaluating the QP solver
    g0 = constr(x)  # Initial value of constraints

    if lbg is None:
        lbg = -np.inf * np.ones(g0.shape)

    if ubg is None:
        ubg = np.inf * np.ones(g0.shape)

    x, f, g, g0, lbg_sym, ubg_sym = map(
        _as_casadi_array,
        (x, f, g, g0, lbg_sym, ubg_sym),
    )
    ...
    linearize = cs.Function("linearize_nlp",
        [x, lbg_sym, ubg_sym],
        [Q, c, A, lba, uba],
        ["x", "lbg", "ubg"],
        ["Q", "c", "A", "lba", "uba"],
    ...
    solver = cs.conic("conic_solver", "osqp", qp, options)

    # Evaluate the solver
    x0, lbg, ubg = map(_as_casadi_array, (x0, lbg, ubg))
    Q, c, A, lba, uba = linearize(x0, lbg, ubg)
    kwargs = {
        "h": Q,
        "a": A,
        "g": c,
    ...
    # Add warm-start variables if provided
    if lam_a0 is not None:
        kwargs["lam_a0"] = _as_casadi_array(lam_a0)

    if x_guess is not None:
        kwargs["x0"] = _as_casadi_array(x_guess)
    ...
    lam_a = array(sol["lam_a"], dtype=ret_dtype).reshape(g.shape)
