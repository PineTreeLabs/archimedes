...
from .eos import R_IDEAL, ThermalFluidEoS
...
class TabulatedPerfectGas(PerfectGasEoS):
    ...
    T_data: np.ndarray = struct.field(
        static=True
    )  # Lookup table temperature values [K]
    h_data: np.ndarray = struct.field(
        static=True
    )  # Lookup table enthalpy values [J/mol]
    cp_data: np.ndarray = struct.field(
        static=True
    )  # Lookup table specific heat capacity values [J/mol-K]
...
class PerfectGasMixture(PerfectGasBase):
    ...
    def __post_init__(self):
        ...
        mole_frac = {gas: frac / den for gas, frac in self.composition.items()}
        ...
        mass_frac = {
            gas: frac * self.database[gas].M / M for gas, frac in mole_frac.items()
        }
        object.__setattr__(self, "mass_frac", mass_frac)

        # Calculate mole-weighted standard enthalpy of formation
        h0_f = self._mole_avg(
            {gas: self.database[gas].h0_f for gas in self.composition}
        )
    ...
    def _mass_prop_avg(self, attr: str, **kwargs) -> float:
        ...
        qty = {
            gas: getattr(self.database[gas], attr)(**kwargs) for gas in self.composition
        }
