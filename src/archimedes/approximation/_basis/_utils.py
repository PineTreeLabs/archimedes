...
def _reference_breakpoints(breakpoints) -> tuple[float, float, np.ndarray]:
    r"""Map physical-domain ``breakpoints`` (spanning ``[a, b] =
...
def _as_mx(value):
    ...
    MX-only. A constant must be promoted from DM before it can be indexed
...
def _locate(knots, x, symbolic: bool, side: str = RIGHT):
    ...
    node provenance already identifies the owning element, as it does for
    quadrature nodes, that provenance is used directly instead, and no such
