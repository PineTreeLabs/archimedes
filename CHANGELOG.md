...
- `LagrangeBasis` supports derivatives of any order (previously only 0 and 1, which left `stiffness_matrix` unusable for Lagrange-based spaces)
- Added `Basis.evaluate_expansion`, which fuses evaluation with the coefficient contraction; `PiecewiseBasis` overrides it with a locate-and-gather that is independent of the element count
- `Function * Function` gives the pointwise product, exactly, in an automatically enlarged space (`n_1 + n_2 - 1`); use `FunctionSpace.project` to reduce back down
