...
from archimedes.approximation import FourierBasis, FunctionSpace, MonomialBasis
...
# -- FunctionSpace.monomial() constructor --


def test_monomial_constructor():
...
def test_derivative_matches_finite_difference_and_closed_form(deriv):
    ...
    np.testing.assert_allclose(dphi, (hi - lo) / (2 * h), atol=1e-3)
...
def test_derivative_on_mapped_domain():
    a, b = 2.0, 7.0
    basis = MonomialBasis(5)
