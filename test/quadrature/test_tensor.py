...
def test_shapes_and_counts():
    rule = tensor_quad(_gl(3), _gl(4), _gl(2))
    assert rule.ndim == 3
    assert len(rule) == 24
...
def test_equality_and_hash():
    assert tensor_quad(_gl(3), _gl(4)) == tensor_quad(_gl(3), _gl(4))
    assert tensor_quad(_gl(3), _gl(4)) != tensor_quad(_gl(4), _gl(3))
    assert tensor_quad(_gl(3), _gl(4)) != tensor_quad(_gl(3), _gh(4))
    assert tensor_quad(_gl(3)) != _gl(3)
    assert hash(tensor_quad(_gl(3), _gl(4))) == hash(tensor_quad(_gl(3), _gl(4)))
...
def test_composite_per_dimension():
    ...
    # a rectilinear mesh is `tensor_quad(composite_quad(...), composite_quad(...))`.
    breaks = np.linspace(-1.0, 1.0, 4)
    rule = tensor_quad(composite_quad(_gl(2), breaks), _gl(3))
...
def test_elements_mirrors_nodes():
    ...
    composite_rule = composite_quad(_gl(2), breaks)  # 2 elements x 2 nodes
    rule = tensor_quad(composite_rule, _gl(3))
...
def test_elements_zero_fills_non_composite_dimensions():
    breaks = np.linspace(-1.0, 1.0, 3)
    rule = tensor_quad(_gl(2), composite_quad(_gl(2), breaks))
...
def test_constructor_accepts_an_explicit_tuple():
    assert TensorQuadratureRule((_gl(3), _gl(2))) == tensor_quad(_gl(3), _gl(2))
