...
from .conftest import assert_positive_definite, compute_nees, compute_nis
...
class TestBenchmarkProblems:
    ...
    def test_van_der_pol_oscillator(self):
        ...
        for filter_name, filter_step in [("EKF", ekf_step), ("UKF", ukf_step)]:
            ...
            assert np.max(positions) > 0.5, f"{filter_name} oscillation amplitude too small"
            assert np.min(positions) < -0.5, f"{filter_name} oscillation amplitude too small"
    ...
    def test_coordinated_turn_model(self):
        ...
        for filter_name, filter_step in [("EKF", ekf_step), ("UKF", ukf_step)]:
            ...
            radii = np.sqrt((positions[:, 0] - center_x)**2 + (positions[:, 1] - center_y)**2)
    ...
    def test_pendulum_system(self, filter_name):
        ...
        assert energy_drift < 0.5, f"{filter_name} energy drift too large: {energy_drift}"
...
class TestRobustness:
    ...
    def test_missing_measurements(self):
        ...
        for filter_name, filter_step in [("EKF", ekf_step), ("UKF", ukf_step)]:
            ...
            for i in range(30):
                ...
                assert_positive_definite(P, f"{filter_name} covariance with missing data")
    ...
    def test_measurement_outliers(self):
        ...
        for filter_name, filter_step in [("EKF", ekf_step), ("UKF", ukf_step)]:
            ...
            assert np.mean(np.abs(outlier_innovations)) > 2*np.mean(np.abs(normal_innovations)), \
    ...
    def test_poor_initial_conditions(self):
        ...
        for filter_name, filter_step in [("EKF", ekf_step), ("UKF", ukf_step)]:
            ...
            for i in range(50):
                ...
                assert_positive_definite(P, f"{filter_name} covariance during convergence")
    ...
    def test_near_singular_covariance(self):
        ...
        for filter_name, filter_step in [("EKF", ekf_step), ("UKF", ukf_step)]:
            ...
            for i in range(30):
                ...
                assert np.all(np.isfinite(x)), f"{filter_name} state with poor conditioning"
                assert np.all(np.isfinite(P)), f"{filter_name} covariance with poor conditioning"
    ...
    def test_consistency_statistics(self, filter_type):
        ...
        assert abs(nees_mean - expected_nees) < 0.5, \
            f"{filter_type.upper()} NEES inconsistent: {nees_mean} vs expected {expected_nees}"
        
        assert abs(nis_mean - expected_nis) < 0.3, \
            f"{filter_type.upper()} NIS inconsistent: {nis_mean} vs expected {expected_nis}"
