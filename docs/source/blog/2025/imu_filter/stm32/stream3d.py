...
def parse_imu_data(line):
    ...
    try:
        ...
        match = re.search(r'Roll:\s*(-?\d+)\s+Pitch:\s*(-?\d+)\s+Yaw:\s*(-?\d+)', 
                        line.decode('utf-8'))
...
def main(port=None, save=None):
    ...
    with serial.Serial(port, 115200, timeout=5) as ser:
        ...
        rpy_display = scene.visuals.Text('Roll: 0.0°  Pitch: 0.0°  Yaw: 0.0°',
        ...
        def update(event):
            """Called at regular intervals to read serial data and update visualization."""
            try:
                ...
                if latest_line:
                    ...
                    if R is not None:
                        ...
                        rpy_display.text = f'Roll: {roll:6.1f}°  Pitch: {pitch:6.1f}°  Yaw: {yaw:6.1f}°'
