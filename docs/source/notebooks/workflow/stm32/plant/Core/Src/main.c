...
/* USER CODE BEGIN PV */
volatile uint32_t adc1_dma_buffer[ADC1_CHANNELS];  // DMA target for ADC1 data
volatile uint16_t dac1_dma_buffer[1];  // DMA target for DAC1 data
volatile uint16_t dac2_dma_buffer[1];  // DMA target for DAC2 data

// Archimedes-generated structs
plant_arg_t plant_u;  // Inputs
plant_res_t plant_y;  // Outputs
...
int main(void)
{
  ...
  // Set up plant model
  plant_init(&plant_u, &plant_y, &plant_w);
  ...
  if (HAL_DAC_Start_DMA(&hdac, DAC_CHANNEL_1, (uint32_t*)dac1_dma_buffer, 1, DAC_ALIGN_12B_R) != HAL_OK) {
      Error_Handler();
  }

  // HAL_DAC_Start_DMA(DAC_HandleTypeDef *hdac, uint32_t Channel, const uint32_t *pData, uint32_t Length, uint32_t Alignment)
  if (HAL_DAC_Start_DMA(&hdac, DAC_CHANNEL_2, (uint32_t*)dac2_dma_buffer, 1, DAC_ALIGN_12B_R) != HAL_OK) {
      Error_Handler();
  }
  ...
  // HIGH = no trigger (simulate button open)
  ...
  plant_u.ENA = 1;
  plant_u.ENB = 1;
  plant_u.pwm_duty = 0;  // Motor off to start
  ...
  while (1)
  {
    ...
    // Onboard button sends 100 ms "trigger" pulse and resets the simulated position
	  if(HAL_GPIO_ReadPin(Onboard_Button_GPIO_Port, Onboard_Button_Pin) == GPIO_PIN_SET) {
        // Reset the state
        for (int i = 0; i < sizeof(plant_u.state)/sizeof(plant_u.state[0]); i++) {
            plant_u.state[i] = 0;
            plant_y.new_state[i] = 0;
        }

        HAL_GPIO_WritePin(Onboard_LED_GPIO_Port, Onboard_LED_Pin, GPIO_PIN_SET);

        // LOW = trigger (simulate button press)
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    if (htim->Instance == TIM6) {
        ...
        plant_u.pwm_duty = ((float)adc1_dma_buffer[0]) / ADC_RESOLUTION;  // ADC1_IN3 (Rank 1) - PWM input
        // adc1_dma_buffer[1];  // ADC1_IN10 (Rank 2) - UNUSED

        plant_u.INA = HAL_GPIO_ReadPin(Driver_INA_GPIO_Port, Driver_INA_Pin);
        plant_u.INB = HAL_GPIO_ReadPin(Driver_INB_GPIO_Port, Driver_INB_Pin);
        plant_u.ENA = HAL_GPIO_ReadPin(Driver_ENA_GPIO_Port, Driver_ENA_Pin);
        plant_u.ENB = HAL_GPIO_ReadPin(Driver_ENB_GPIO_Port, Driver_ENB_Pin);

        /* SIMULATION */

        // Step the plant model forward
        plant_step(&plant_u, &plant_y, &plant_w);

        // Copy state from output struct back to inputs for next step
        for (int i = 0; i < sizeof(plant_u.state)/sizeof(plant_u.state[0]); i++) {
            plant_u.state[i] = plant_y.new_state[i];
        }

        /* OUTPUTS */

        // Store the simulated voltage outputs in the DAC buffers
        dac1_dma_buffer[0] = (uint16_t)(plant_y.V_CS / ADC_TO_VOLTS);  // Simulated V_CS
        dac2_dma_buffer[0] = (uint16_t)(plant_y.VOUTA / ADC_TO_VOLTS);  // Simulated VOUTA
