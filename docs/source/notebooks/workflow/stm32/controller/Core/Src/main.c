...
volatile uint32_t adc3_dma_buffer[ADC3_CHANNELS];  // DMA target for ADC3 data

uint32_t cs_raw = 0;
uint16_t pwm_duty = 0;
float pos_deg = 0.0f;  // DEBUG: Shouldn't need this
...
controller_arg_t controller_u;  // Inputs
controller_res_t controller_y;  // Outputs
controller_work_t controller_w;  // Workspace
...
int main(void)
{
  ...
  controller_init(&controller_u, &controller_y, &controller_w);
  ...
  // Configure motor direction (forward)
  /*
  Reference:
  |--INA--|--INB--|--STATE--|
  | HIGH  |  LOW  | FORWARD |
  ...
  */
  ...
  while (1)
  {
    ...
    controller_u.pos_ref = (sample_idx < SAMPLE_COUNT / 2) ? 1.0 : 0.0;
...
void controller_callback(void) {
    ...
    cs_raw = adc1_dma_buffer[1];  // ADC1_IN10 (Rank 2) - current sense resistor
    vout_a_raw = adc3_dma_buffer[0];  // ADC3_IN13 (Rank 1) - output voltage A
    vout_b_raw = adc3_dma_buffer[1];  // ADC3_IN9 (Rank 2) - output voltage B
    ...
    controller_u.pos = pos_deg / RAD_TO_DEG;
    // controller_u.pos = 0.0f;
    controller_u.i_motor = (float)cs_raw * CS_SCALE;  // Convert ADC counts to amperage
    
    /* CONTROLLER LOGIC */
    controller_step(&controller_u, &controller_y, &controller_w);
    
    // Copy state from output struct back to inputs for next step
    for (int i = 0; i < sizeof(controller_u.x)/sizeof(controller_u.x[0]); i++) {
        controller_u.x[i] = controller_y.x_new[i];
    }

    /* OUTPUTS */
    // Update PWM duty cycle
    pwm_duty = (uint16_t)(PWM_COUNT * controller_y.pwm_duty);
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, pwm_duty);

    // Set motor driver logic for direction
    if (controller_y.INA == 0) {
    ...
    if ((loop_count % sample_rate == 0) && (sample_idx < SAMPLE_COUNT)){
      ...
      v_motor_mv[sample_idx] = (int32_t)((vout_a_raw - vout_b_raw) * VOUT_SCALE * 1000);  // millivolts
      i_motor_ma[sample_idx] = (uint16_t)(controller_u.i_motor * 1000);  // milliamps
      // pos_mdeg_int[sample_idx] = (int32_t)(controller_u.pos * RAD_TO_DEG * 1000);  // millidegrees
...
void send_data(void) {
    ...
    snprintf(header, sizeof(header), "START,%lu,%d,%u,%u,%lu\n",
            (uint32_t)SAMPLE_COUNT, 4, PWM_COUNT, sample_rate, loop_cycles / SAMPLE_COUNT);
