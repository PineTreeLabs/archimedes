...
def _wait_for_start_message(ser):
    ...
    while True:
        ...
        if line.startswith("START"):
            sample_count, loop_cycles = map(int, line.split(",")[1:])
            print(f"Receiving {sample_count} samples")
            if loop_cycles > 0:
                # Divide by 480 MHz to get execution time in µs
                print(f"Control loop time: {loop_cycles / sample_count / 480:.4f} µs")
            return sample_count
