...
typedef struct
{
  ...
  int (*read)(void *handle, uint8_t dev, uint8_t reg, uint8_t *data, uint16_t len);
  int (*write)(void *handle, uint8_t dev, uint8_t reg, uint8_t *data, uint16_t len);
} lsm6dsox_dev_t;
...
static inline int lsm6dsox_init(lsm6dsox_dev_t *dev)
{
  ...
  // Check WHO_AM_I
  uint8_t whoami;
  if (dev->read(dev->handle, dev->addr, LSM6DSOX_WHOAMI, &whoami, 1) != 0)
  {
    return -1;
  }
  ...
  dev->write(dev->handle, dev->addr, LSM6DSOX_CTRL1_XL, &ctrl1, 1);
  uint8_t ctrl2 = lsm6dsox_get_ctrl2(dev->gyro_odr, dev->gyro_range);
  dev->write(dev->handle, dev->addr, LSM6DSOX_CTRL2_G, &ctrl2, 1);
  uint8_t ctrl8 = lsm6dsox_get_ctrl8(false, LSM6DSOX_HPF_ODR_DIV_4); // No HPF
  dev->write(dev->handle, dev->addr, LSM6DSOX_CTRL8_XL, &ctrl8, 1);
    ...
    const lsm6dsox_dev_t *dev, lsm6dsox_data_t *data)
{
  ...
  int ret = dev->read(dev->handle, dev->addr,
                      LSM6DSOX_OUT_TEMP_L, data->buffer, 14);
  if (ret != 0)
    return ret;
