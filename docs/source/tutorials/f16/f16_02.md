...
```{code-cell} python
import yaml
from f16 import SubsonicF16Config

with open("config.yaml", "r") as f:
    config_dict = yaml.safe_load(f)

stability_model = SubsonicF16Config.model_validate(config_dict["stability"]).build()
pprint(stability_model)
```

```{code-cell} python
full_model = SubsonicF16Config.model_validate(config_dict["full"]).build()
pprint(full_model)
```
...
However, as with the hierarchical design itself, in your own application you may find the configuration management system to be either helpful and scalable, or complicated overkill - in either case you are probably correct.
