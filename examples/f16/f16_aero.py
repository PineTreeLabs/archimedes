...
class F16Aerodynamics:
    ...
    def __call__(self, vt, alpha, beta, w_B, el, ail, rdr, vehicle: SubsonicF16):
        ...
        dail = ail / 20.0
        drdr = rdr / 30.0
        clt = self._calc_cl(alpha_deg, beta_deg) + dlda(alpha_deg, beta_deg) * dail + dldr(alpha_deg, beta_deg) * drdr
        cmt = self._calc_cm(alpha_deg, el)
        cnt = self._calc_cn(alpha_deg, beta_deg) + dnda(alpha_deg, beta_deg) * dail + dndr(alpha_deg, beta_deg) * drdr
        ...
        cnt = cnt + b2v * (d[7] * r + d[8] * p) - cyt * (vehicle.xcgr - vehicle.xcg) * vehicle.cbar / vehicle.b
