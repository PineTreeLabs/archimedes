...
def balanced_truncation(A, B, C, D=None, r=None, tol=1e-5):
    ...
    if D is None:
        ...
        if not (D.ndim == 2 and D.shape[0] == C.shape[0] and D.shape[1] == B.shape[1]):
            raise ValueError("D must have shape (p, m) where p is the number of rows in C and m is the number of columns in B")
    
    # Check system stability before proceeding
    eigs = linalg.eigvals(A)
    if np.max(np.real(eigs)) >= 0:
        raise ValueError("System is not stable. Balanced truncation requires a stable system.")
